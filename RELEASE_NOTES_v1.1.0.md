...
After extensive beta testing, BacaPetra v1.1.0 is now officially released as a
fully-production-ready application! This version transforms the app
from beta status to a fully-featured reading platform with significant new capabilities.
...
- **Persistent Settings**: Font size preferences are saved and restored across
app sessions
...
- **Fixed Critical Bug**: Resolved `FontSize.large!.value` error that prevented
font scaling
- **Proper Font Sizing**: Implemented correct font size calculation using
`FontSize(18.0 * fontSizeScale)`
- **Cross-Platform Compatibility**: Ensured font scaling works on all supported
devices
...
- **New Provider**: Added `FontSizeProvider` for centralized font scaling state
management
